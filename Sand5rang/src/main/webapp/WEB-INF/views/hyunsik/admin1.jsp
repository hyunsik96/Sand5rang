		...
		<div class="container">
			...
			<ul class="mainnav">
				...
				<li class="dropdown active">					
					...
					<ul class="dropdown-menu">
                        <li class="active"><a href="ad1.hs">가맹점 주문처리</a></li>
    ...
    <c:forEach var="indent" items="${list[status.index]}" varStatus="status2">
	    ...
	    <c:if test="${indent.ingNo eq 24}">
			<c:set var="x" value="${indent.count}"/>
             ...
             <table class="table table-striped table-bordered">
                  <tr>
                    <td class="td-actions"><span class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" data-target="#${status.index}1, #${status.index}2, #${status.index}3, #${status.index}4, #${status.index}5">${aaa[0].storeName}</span></td>
...
<c:if test="${empty list}"><div id="micheuri">가맹점의 미처리 주문내역이 없습니다.</div></c:if>
				</div> <!-- /widget -->
...
<c:choose>
	...
	<c:otherwise>
	<li class="page-item"><a class="page-link" href="ad1.hs?p=${pi.currentPage - 1}"><</a></li>
	</c:otherwise>
</c:choose>
