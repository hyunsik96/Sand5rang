...
<c:if test="${empty list}"><div id="micheuri">가맹점별 대금내역이 없습니다.</div></c:if>
				</div> <!-- /widget -->
...
<c:choose>
	...
	<c:otherwise>
	<li class="page-item"><a class="page-link" href="ad3.hs?p=${pi.currentPage - 1}"><</a></li>
	</c:otherwise>
</c:choose>

<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}">
	<c:if test="${pi.currentPage eq p}">
		<li class="page-item active"><a class="page-link" href="ad3.hs?p=${p}">${p}</a></li>
	</c:if>
	<c:if test="${pi.currentPage ne p}">
		<li class="page-item"><a class="page-link" href="ad3.hs?p=${p}">${p}</a></li>
	</c:if>
</c:forEach>
